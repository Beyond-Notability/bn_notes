...
```{r finishing-up}
# finishing up: checked.  RData notes_notes-on-dates
```
...
bind_rows(
  ...
  filter(n==2) |>
  arrange(bn_id) |>
  select(bn_id, date_propLabel, year) |>
  ...
  mutate(bn_date_id = case_when(
    date_propLabel=="date of birth" ~ 1,
    date_propLabel=="date of death" ~ l+1,
    .default = bn_date_id
  )) |>
...
bn_women_dates_categories |>
  mutate(subcat = case_when(
    date_propLabel %in% c("date of birth", "date of death") ~ "birth/death",
...
bn_women_dates_birth_death |>
  left_join(bn_women_dates_subcat |> ##select(-date_propLabel) 
              filter(!is.na(subcat)) |>
...
#   filter(date_propLabel != "date of birth") |>
#   # mutate(age = year-year_birth) |>
#   # mutate(period = case_when(
...
- hover over points for more info in tooltips
...
reactable(
  bn_women_dates_birth_death |>
  select(-person, -bn_date_id, -year_birth, -year_death, -prop_value, -s, -date_pit_precision, -qual_date_prop) ,
  filterable = TRUE,
  searchable = TRUE,
  bordered = TRUE,
  highlight = TRUE,
  ...
  columns = list(
    ...
    date_propLabel = colDef(maxWidth = 120, name="property",
      html = TRUE,
      cell = function(value, index) {

...
format:
  revealjs:
    footer: "<a href='http://sharonhoward.org/bn/bn_notes/'>Home</a>"
execute: 
  echo: false
  warning: false
  message: false
...
# nb that the imputed number for men in 1920 looks slightly questionable, but it'll do for now.
  # across() works with na.spline but if i can't work out how to do round at the same time there's not much point...
...
# hmm current query gets quite a few more results than above...
...
# proposed_summary <-
#   proposed_sal |>
#   distinct(person, personLabel, date, proposed, proposedLabel) |>
#   # add count to check for women who tried twice
#   add_count(person, name="n_elections") |>
#   # add proposer gender
#   left_join(proposed_sal2 |> distinct(proposer, proposerGender), by=c("proposed"="proposer")) |>
#   # add outcome
#   inner_join(proposed_sal2 |> 
#                distinct(FSA, date, elected), by=c("person"="FSA", "date"))  |>
#   # fix missing outcome: assume success, for now
#   mutate(elected = if_else(is.na(elected), "successful", elected))  |>
#   # add count of signers
#   left_join(proposed_sal |> 
#               count(person, date, name="n_signer"), by=c("person", "date")) |>
#   # add count of signers by gender. _n is NA (mostly indecipherable). checked they match n_signer.
#   left_join(proposed_sal |>
#               count(person, date, signgender) |>
#               pivot_wider(names_from = signgender, values_from = n, values_fill = 0) |>
#               rename(sign_m=man, sign_f=woman, sign_n=`NA`), by=c("person", "date")) |>
#   # add fellows yearly numbers from list with imputed missing values
#   mutate(year = year(date)) |>
#   #inner_join(bn_fellows_lists |> select(year, women_imp, total_imp, p_women_imp, yr), by="year") |>
#   relocate(year, n_elections, .after = date) |>
...
# 
# ## proposers
# proposers_dates_summary <-
# proposer_summary |>
#   # filter female proposers
#   filter(proposerGender=="woman") |>
#   select(person=proposed, name=proposedLabel, n_proposal) |>
#   # get proposer's own election data - make sure it's when they were elected
#   inner_join(proposed_summary |> filter(elected=="successful") |>
#                select(person, election_date=date, election_year=year, proposer=proposed, proposerGender), by=c("person")) |>
#   arrange(election_date) |>
#   inner_join(
#     proposed_summary |>
#       group_by(proposed) |>
#       summarise( earliest_proposal = min(date), earliest_year = min(year), latest_proposal=max(date)) |>
#       ungroup(), by=c("person"="proposed")
#   )
# 
# ## signers
...
# signer_summary |>
#   filter(signgender=="woman") |>
#   select(person=signatorypersonal, name=signatorypersonalLabel, n_sign) |>
#   # get signer's election data
#   inner_join(proposed_summary |> filter(elected=="successful") |>
#                select(person, election_date=date, election_year=year, proposer=proposed, proposerGender), by=c("person")) |>
#   arrange(election_date) |>
...
#       group_by(SALsignatorypersonal) |>
#       summarise(earliest_sign = min(date), earliest_year = min(year(date)), latest_sign=max(date)) |>
#       ungroup(), by=c("person"="SALsignatorypersonal")
#   )
# ptol vibrant: orange '#EE7733', cyan '#33BBEE', grey '#BBBBBB'

...
bn_suffrage_query |>
  ...
  # a `recorded in` has appeared which is unlikely to be relevant. 
  # might need to keep an eye on any other unexpected NAs though!
  # where the f has this chartiable thing come from?
  filter(!is.na(bucket)) |>
  filter(!bucket %in% c("charitable/organisational")) |> 
...
bn_suffrage |>
  # drop publishing
  filter(!bucket %in% c("publishing", "charitable/organisational")) |> 
  left_join(

...
This was more fiddly than I expected so I've only done a few UK regions with small numbers of excavations so far. I'll see if I can streamline it and do some more later.

NB that dates are completely ignored: it's just mapping the location of an excavation with all locatable resided_at locations for participants. Excavations that don't have any locatable participants are omitted.

Maps start by showing all the excavations and people for the region and then you can use the checkboxes to filter in/out individual excavations. I could do it with radio buttons to show just one excavation at a time but I quite like the overview.

Also I'm not sure if I can get clustering options to work, so at present you can only see one circle if there are several at the same location. Filtering will help sometimes. (If there's an excavation and a person at the same spot there is a slight difference in colour.)
...
myLegend <- function(m) {
  addLegend(
    ...
    position = "topright",
    colors = pal_colours,
  labels = c("excavation", "participant"), 
  opacity = 1,
  title = "circle type"
  )
}
...
bn_excavations_regions |> 
  filter(regionLabel %in% c("North West England", "North East England")) |>
...
bn_participants_residences |> 
  ...
  mutate(excavation_group=glue("{excavationLabel} {excavation}")) |>
  mutate(popup = glue("{personLabel}", "{excavationLabel}", "{roleLabel}", "{locationLabel}", .sep="<br/>")) |>
...
leaflet(
   ...
   addTiles() |> # modern base layer 
  # add using [1][[1]] instead of names.
   addCircleMarkers(data = list_of_scotland_excavations[1][[1]], lat = ~lat, lng=~long, layerId = ~id, radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |>
   addCircleMarkers(data = list_of_scotland_excavations[2][[1]], lat = ~lat, lng=~long, layerId = ~id, radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |>
   addCircleMarkers(data = list_of_scotland_excavations[3][[1]], lat = ~lat, lng=~long, layerId = ~id,radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |>
   addCircleMarkers(data = list_of_scotland_excavations[4][[1]], lat = ~lat, lng=~long, layerId = ~id, radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |>
  
 # addCircleMarkers(data = list_of_north_england_excavations[8][[1]], lat = ~lat, lng=~long, layerId = ~id, radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |> # can't have too many.
  
   addLayersControl(
    overlayGroups = bn_excavations_scotland$excavation_group, # assume will only work if you have all the groups! otherwise you have to do stuff like c("Q1856", "Q3717"),
    options = layersControlOptions(collapsed = FALSE)
  ) |>
  myLegend()
...
leaflet(
   ...
   addTiles() |> # modern base layer 
  # add using [1][[1]] instead of names.
   addCircleMarkers(data = list_of_wales_excavations[1][[1]], lat = ~lat, lng=~long, layerId = ~id, radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |>
   addCircleMarkers(data = list_of_wales_excavations[2][[1]], lat = ~lat, lng=~long, layerId = ~id, radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |>
   addCircleMarkers(data = list_of_wales_excavations[3][[1]], lat = ~lat, lng=~long, layerId = ~id,radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |>
   addCircleMarkers(data = list_of_wales_excavations[4][[1]], lat = ~lat, lng=~long, layerId = ~id, radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |>
  addCircleMarkers(data = list_of_wales_excavations[5][[1]], lat = ~lat, lng=~long, layerId = ~id, radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |>
  addCircleMarkers(data = list_of_wales_excavations[6][[1]], lat = ~lat, lng=~long, layerId = ~id, radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |>
  addCircleMarkers(data = list_of_wales_excavations[7][[1]], lat = ~lat, lng=~long, layerId = ~id, radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |>
  addCircleMarkers(data = list_of_wales_excavations[8][[1]], lat = ~lat, lng=~long, layerId = ~id, radius = ~ifelse(type == "person", 6, 8), group = ~excavation_group, popup = ~popup, color=~pal(type), opacity = 0.6, fillOpacity = 0.3 ) |> # can't have too many.
  
   addLayersControl(
    overlayGroups = bn_excavations_wales$excavation_group, # assume will only work if you have all the groups! otherwise you have to do stuff like c("Q1856", "Q3717"),
    options = layersControlOptions(collapsed = FALSE)
  ) |>
  myLegend()

...
```{r finishing-up}
# finishing up: checked. RData  served_on-2023-11-05 adjusted some charts appearance, minor text update.
```
...
There are about 100 different organisations, about half of which are represented by a single service event.

```{r}
bn_women_served_service |>
  count(of_label, sort = T) |>
...
#  count(serviceLabel) |>
  mutate(type = case_when(
    ...
    str_detect(serviceLabel, "British Academy Archaeological and Historical") ~ "Archaeological",
    serviceLabel %in% c("Council") ~ serviceLabel,
    str_detect(serviceLabel, "Committee$") ~ str_extract(serviceLabel, "\\w+(?= Committee)"),
...
bn_women_served_service |>
  ...
  #theme(legend.position = "bottom") +
  theme(legend.position.inside = c(0.7, 0.5)) +

...
bn_events_fetched <-
if(file.exists(bn_events_csv_file))  read_csv(bn_events_csv_file) else  
  # otherwise fetch the sparql query and save to csv and then read the csv. 
  bn_query_to_csv(bn_women_events_sparql, bn_events_csv_file)
...
'SELECT distinct ?person ?prop ?ppa  ?date ?date_precision ?date_prop ?date_propLabel ?pqv  ?s
...
WHERE {  
  ...
  # check number of results but this seems to work 
        ?s ?pq ?date .   
...
if(file.exists(bn_events_time_precision_csv_file))  read_csv(bn_events_time_precision_csv_file) else  
  # otherwise fetch the sparql query and save to csv and then read the csv. 
  bn_query_to_csv(bn_women_events_time_precision_sparql, bn_events_time_precision_csv_file)
...
#   mutate(event_type = case_when(
#     event %in% c("meeting", "exhibition", "conference") ~ event,
#     event_id=="Q292" & is.na(of_value) ~ "meeting",  # folklore society not specified as meetings, but they almost certainly are
#     #event_id=="Q682" ~ "conference", # Annual Meeting as conference? - to work this has to go before instance
#     instance %in% c("meeting", "exhibition", "conference") ~ instance,
#     event %in% c("committee", "museum") ~ "other",
#     str_detect(event, "Meeting|Congress of the Congress of Archaeological Societies") ~ "meeting",
#     str_detect(event, "Conference|Congress") | str_detect(of_value, "Conference|Congress") ~ "conference",
#     #str_detect(instance2, "society|organisation|museum|institution|library") ~ "other",
#     str_detect(of_value, "Society|Museum|Library|Institut|Association|School|College|Academy|University|Club|Gallery|Committee") | str_detect(event, "Society|Museum|Museo|Library|Institut|Association|School|College|Academy|University|Club|Gallery|Committee") ~ "other",
#     .default = "misc"
#   )) |>
#   
#     mutate(event_org = case_when(
#     !is.na(of_value) ~ of_value,
#     event_id=="Q292" & is.na(of_value) ~ event,
#     event_type=="other" ~ event,
#     str_detect(event, "Royal Archaeological Institute|\\bRAI\\b") ~ "Royal Archaeological Institute", 
#     str_detect(event, "Society of Antiquaries of London|\\bSAL\\b") ~ "Society of Antiquaries of London",
#     str_detect(event, "Congress of Archaeological Societies|\\bCAS\\b") ~ "Congress of Archaeological Societies",
...
#   )) |>
# 
#   # need an org id as well as org name.
#   mutate(org_id = case_when(
#     !is.na(of_id) ~ of_id,
#     event_id=="Q292" & is.na(of_value) ~ event_id,
#     # need these IDs 
#     str_detect(event, "Royal Archaeological Institute|\\bRAI\\b") ~ "Q35", 
#     str_detect(event, "Society of Antiquaries of London|\\bSAL\\b") ~ "Q8",
#     str_detect(event, "Congress of Archaeological Societies|\\bCAS\\b") ~ "Q186", 
#     str_detect(event, "Royal Academy") ~ "Royal_Academy",
#     str_detect(event, "Society of Lady Artists") ~ "Q1891",
#     str_detect(event, "Folklore Society") ~ "Q292",
#     !is.na(event_org) ~ event_id,
#     # conferences etc without an of - use event_id. but not if generic
#     event_type %in% c("conference", "exhibition", "misc") & !event %in% c("meeting", "exhibition", "event", "petition", "conference") ~ event_id
...
#   mutate(event_title = case_when(
#     # for FS
#     event_id=="Q292" & is.na(of_value) ~ paste("meeting,", event),
#     #  use year if other info is lacking. either should match instance id without a problem 
#     event %in% c("exhibition", "meeting", "event", "conference") & is.na(of_value) & !is.na(year) ~ paste0(event, " (", year, ")"),
#     is.na(of_value) ~ event,
#     event=="event" ~ of_value,
#     .default = paste(event, of_value, sep=", ")
#   )) |>
#   # some abbreviations
#   mutate(event_title = str_replace_all(event_title, sal_rai_cas_abbr))  |>
# 
...
#   # mutate(event_of_id = paste(event_id, of_id, sep="_")) |>
#   mutate(event_instance_id = paste(event_instance_date, org_id, event_type, sep="_"))  |>
#   # hmm, this may not quite work. might need a bit of extra work for CAS etc. 
#   mutate(event_org_id = paste(org_id, event_type, sep="_")) |>
#   relocate(event_title, event_type, year, event_instance_date, event_org, org_id, event_instance_id, event_org_id, .after = ppa_type) 
#   
# bn_women_events_of_dates_types <-
# bn_women_events_of_dates_types_all |>
#   # losing ppa_label, but keep ppa in case you need any joins. just bear in mind slight difference.
...
# bn_women_event_instances <-
# bn_women_events_of_dates_types_all |>
#   group_by(event_instance_id, event_org_id, event_title, event_type, event_org, event, of_value, event_id, of_id, event_instance_date, year) |>
#   # get all unique dates listed for the event instance, in chronological order
#   arrange(date, .by_group = T) |>
#   summarise(dates_in_db = paste(unique(date), collapse = " | "), .groups = "drop_last") |>
#   ungroup() 
...
#       select(s, qual_location=qual_valueLabel, qual_location_value=qual_value)
#   , by="s") |>
#   # make event type. adjusted to do more as you dropped second i/o join
#   mutate(event_type = case_when(
#     event %in% c("meeting", "exhibition", "conference") ~ event,
#     instance %in% c("meeting", "exhibition", "conference") ~ instance,
#     event %in% c("committee", "museum") ~ "other",
...
#     .default = "misc"
#   )) |>
#   # consolidate ppa_label item/text. currently only delegate
#   mutate(ppa_type = case_when(
#     str_detect(ppa_label, "was delegate") ~ "was delegate at",
#     .default = ppa_label
#   )) |>
#   relocate(ppa_type, event_type, .after = ppa)  |>
#   # grouping date for distinct events according to type of event... workings/rules further down this doc.
#   # do i need to check this again after adjusting event_type? i think it's ok. only a handful changed and even fewer made a difference to the group date. only change to general rules other=ymd. 
#   mutate(event_instance_date = case_when(
#     is.na(date) ~ NA,
#     event_id=="Q682" ~ paste0(year, "-01-01"),
#     event_type %in% c("misc", "meeting", "other") ~ as.character(date),
#     event_type %in% c("conference", "exhibition") ~ paste0(year, "-01-01")
#   ))  |>
#   # and id columns for convenience
#   mutate(event_instance_id = paste(event_instance_date, event_id, of_id, sep="_"))  |>
#   mutate(event_of_id = paste(event_id, of_id, sep="_")) |>
#   # to ignore the free text of/event IDs but keep NA of. drops about 11 rows.
#   #filter(str_detect(event_of_id, "Q\\d.*_(Q|NA)")) |>
#   # event title... wip.
#   mutate(event_title = case_when(
#     # if there's a location and no other specific info, use it. hmm but how to ensure instance id matches. 
#     # you could use year (or even instance date). either should match instance id without a problem
#     #!is.na(qual_location) & event %in% c("exhibition", "meeting", "event") & is.na(of_value) ~ paste0(event, " (", qual_location, ")"),
#     # fix flipping Gomme
#     event %in% c("exhibition", "meeting", "event", "conference") & is.na(of_value) & !is.na(year) ~ paste0(event, " (", year, ")"),
#     is.na(of_value) ~ event,
#     event=="event" ~ of_value,
#     event_type=="meeting" ~ paste0(event, " of ", of_value),
#     .default = paste(event, of_value, sep=", ")
#   )) |>
#   mutate(event_title = str_replace_all(event_title, sal_rai_cas_abbr)) |>
#   mutate(event_org = case_when(
...
#     str_detect(event, "Congress of Archaeological Societies|\\bCAS\\b") ~ "Congress of Archaeological Societies",
#     str_detect(event, "Royal Academy") ~ "Royal Academy",
#     str_detect(event, "Society of Lady Artists") ~ "Society of Lady Artists",
#   )) 
# 
# 
# # this is still a bit WIP, disambiguation may still be improvable
# # unique event instances based on the workings.  
# bn_women_event_instances <-
# bn_women_events_of_dates_types |>
#   group_by(event_instance_id, event_of_id, event, event_type, of_value, event_id, of_id, event_instance_date, event_title, event_org, year) |>
#   # get all unique dates listed for the event instance, in chronological order
#   arrange(date, .by_group = T) |>
#   summarise(dates_in_db = paste(unique(date), collapse = " | "), .groups = "drop_last") |>
#   ungroup() 
...
```{r}
## grouping
# since your brain is very small and pathetic
# 1. distinct event *instances* per woman ("attendances")
# excluding different activities:  distinct(bn_id, personLabel, event_title,  event_type, event_org, event_org_id, org_id, year, event_instance_date, event_instance_id)
# including different activities: distinct(bn_id, personLabel, ppa_type, event_title,  event_type, event_org, event_org_id, org_id, year, event_instance_date, event_instance_id) 
# - can add year to the above but not date.
# 2. distinct *events* per woman . this is not working right at the moment anyway.
# excluding different activities: distinct(bn_id, event_title, event, of_value, event_of_id)
# including different activites: distinct(bn_id, ppa_type, event_title, event, of_value, event_of_id)
```
...
There are basically two different kinds of event in the data: recurring events (eg society monthly meetings or AGMs) and one-off events. Recurring events are a bit more complicated to handle when counting things. So, for recurring events, "event" will refer to any/all instances of (eg) "meeting of the Folklore Society" (Q31 / Q292), "Royal Archaeological Institute Meeting" (Q29), and so on. 

An "event instance" refers to a *single occurrence* of a recurring event (eg the meeting of the Folklore Society that was held on 17 December 1903). For counting one-off events "event" and "event instance" are the same thing.

An "event attendance" refers to one woman participating at one event instance. Sometimes the same event instance appears more than once on a woman's page under different PPAs; unless the emphasis is on the specific activity rather than the event, I'll reduce them to one for counting purposes.
...
```{r}
## Event attendance in life courses?
# problem with interpretation. women *might* be attending events younger later in the period, but it might just be an artefact of data collection.
# bn_women_events_of_dates_types |>
...
#   filter(!is.na(year)) |>
# #   filter(!is.na(yob)) |>
# #   # b. 1840 = 30 in 1870 and probably d. c. 1920. b. 1880 = 30 in 1910 and 70 in 1950.
# #   # filter(between(yob, 1840, 1879)) |>
#   group_by(bn_id, personLabel, yob, birth) |>
#   summarise(n=n(), earliest=min(year), latest=max(year), .groups = "drop_last") |>
#   ungroup() |>
# #   #count(yob<1860) # 1830-89 not quite as even as all, but not far off...
#    filter(n>2) |>
# #   # hang on a minute, what are you actually plotting here?
#    mutate(age_first = earliest-yob, age_last = latest-yob) |>
# #   #count(age_first<40)
# #   mutate(decade_birth = yob - (yob %% 10)) |>
# #   mutate(birth_group = if_else(yob<1860, "a", "b")) |>
# #   filter(between(decade_birth, 1840, 1890)) |>
# #   # if you take out the ends maybe it works? first is getting younger, and that shouldn't be because of data.
# #   # but you need a better way to show it!
# #   filter(between(age_first, 30, 50)) |> 
# #   count(age_first, birth_group, decade_birth) |>
# #   #count(age_first) |>
#    ggplot(aes(age_first)) +
#    geom_histogram(binwidth = 1) +
#    facet_wrap(~birth) +
#    scale_colour_ptol()

# pretty even split around birth year 1860 so let's make two groups
# ok but what about early and late birth decades not being fully represented in teh data? ie if someone was born in 1910s she was only 30-40 at cut off date for project, or born in 1810s were between 30 and 40 at earliest dated event.  
# try dropping any b < 1830 and > 1889 ?
```

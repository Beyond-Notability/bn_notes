...
```{r finishing-up}
# finishing up: checked. RData events-2024-02-26
# ok but messy.!!
# a few edits to text.
```
...
bn_women_events_query |>
  ...
  # drop a handful of <uv> ppas.
  filter(!str_detect(ppa, "^_:t"))
...
# better to get them separately esp as there are multis etc
# problems adapting the query for events only... just get all in ppa
# it's not that slow;just do a semi join afterwards
...
# fetching date_prop makes the query a *lot* slower, so turn prop IDs into labels here 
...
# use consolidated event org_id for locations? yes.
# probably need to prioritise location ahead of any org matches, but worry about that after you've done matching
# for some reason qual_location_value seems to be the ID, not the name...
bn_events_locations <-
bn_women_events_of_dates_types_all |>
  distinct(bn_id, event_org, org_id, location_label=qual_location, location= qual_location_value,  s) 


## which things to use for locations?
## location; event_org id


# pivot longer to put all potential locatables into a single find_loc col
bn_events_loc_id_long <-
  bn_events_locations |>
  pivot_longer(c(location, org_id), names_to = "find_loc_type", values_to = "find_loc_id", values_drop_na = TRUE) |>
  mutate(find_loc_label = case_when(
    find_loc_type=="location" ~ location_label,
    find_loc_type=="org_id" ~ event_org
  )) |>
  # remove any non Q items.
  filter(str_detect(find_loc_id, "^Q\\d")) |>
  distinct(bn_id, find_loc_type, find_loc_id, find_loc_label, s)

# bn_make_union for VALUES
# these must all be Q ids...
bn_events_loc_id_values <-
bn_events_loc_id_long |>
...
bn_events_p12_sparql <-
  bn_events_loc_id_values |>
  mutate_glue_sparql(bn_p12_spql)


bn_events_p12_query <-
  bn_std_query(bn_events_p12_sparql) |>
  make_bn_ids(item)
...
# add locality to work_loc_id
bn_events_loc_id_long_locality <-
bn_events_loc_id_long |>
  left_join(bn_events_p12, by=c("find_loc_id"="item")) 



## set up IDs with io locality for p33 query. 
bn_events_has_locality <-
bn_events_loc_id_long |>
  inner_join(bn_events_p12 |> filter(!is.na(has_locality)) , by=c("find_loc_id"="item")) |>
...
bn_events_p2_query <-
  bn_std_query(bn_events_p2_sparql) |>
...
bn_events_p2_query |>
  ...
  # a few; all are london.
  group_by(find_loc_id) |>
  top_n(1, row_number()) |>
  ungroup()


## check: how many potential locations have been omitted due to absence of P2/P33 info? If there are a lot it may be worth trying wikidata for admin territory/historic county/region data.

# combine p2 with original io locality 
bn_events_p33_ids <-
bind_rows(
  bn_events_p2 |> mutate(src="p2"),
  bn_events_has_locality |> mutate(src="p33")
  ) |>
  arrange(find_loc_id) 


## ADDED CHECK
bn_events_p33_values <-
bn_events_p33_ids |>
...
# # the regions query already exists for this info you dingbat. it's not in the lecturers thing in notes. do you need it at all?
# 
# # spql
bn_p33_spql <-
...
# sparql.
bn_events_p33_sparql <-
  bn_events_p33_values |>
  mutate_glue_sparql(bn_p33_spql)

# query
bn_events_p33_query <-
  bn_std_query(bn_events_p33_sparql) |>
  make_bn_ids(c(item, region))


# add uk regions. dont use, can miss things.
# check that any dropped are non-UK
# check for any multis
# bn_events_p33_ids_regions <-
# bn_events_p33_ids |>
...
bn_events_loc_linked <-
bn_events_loc_id_long_locality |>
  left_join(bn_events_p33_ids |> select(find_loc_id, p33_id, p33_label), by=c("find_loc_id")) |>
  # to prioritise find_loc locations 
  mutate(link_type = case_when(
    find_loc_type=="location" ~ 1,
    .default = 2
  )) |>
  left_join(bn_events_p33_query |> select(item, region, regionLabel), by=c("p33_id"="item"))  |>
  relocate(p33_id, p33_label, region, regionLabel)


bn_events_loc_linked_region <-
bn_events_loc_linked |>
  filter(!is.na(region)) |>
  group_by(s) |>
  top_n(-1, link_type) |>
...
# 176 :-/ are there any left in loc_linked_region?
# bn_events_loc_linked |>
#   filter(!is.na(region)) |>
#   group_by(s) |>
...
# all non UK
# bn_events_p33_ids |>
#   anti_join(bn_regions_localities |> select(-itemLabel, -ioLabel), by=c("p33_id"="item"))

# 116 distinct find_loc_id not located. some not UK
# bn_events_loc_id_long_locality |>
#    anti_join(bn_events_loc_linked |> filter(!is.na(region)), by=c("find_loc_id")) |> count(find_loc_label, find_loc_id)
```
...
NB: "was delegate at (item)" and "was delegate at (free text)" will be merged into a single PPA type "was delegate at" for purposes of analysis.
...
At least 8 attendances (ignoring different PPAs). About 150 women have only a single event attendance; about 50 went to 8 or more.
...
bn_women_events_of_dates_types |>
  ...
  geom_col() +
  scale_fill_ptol() +
  #theme(aspect.ratio = 7/4) + # not doing what you want
...
bn_women_events_of_dates_types |>
 ...
 # filter(n <=12) |> arrange(birth)
  ...
  scale_fill_ptol() +
 # theme(aspect.ratio = 1) + # bad idea w faceted...
...
bn_events_loc_linked_region |>
  ...
  labs(fill="count", title="Heatmap of regional locations of event attendances")
...
bn_events_loc_linked_region |>
  count(regionLabel) |>
  filter(regionLabel != "London (region)") |>
  inner_join(ons_uk_regions_map, by=c("regionLabel"="bn_name")) |>
  ggplot(aes(geometry=geometry, fill=n)) +
  geom_sf(colour="white") +
  coord_sf() +
  scale_fill_viridis_c() +
  labs(fill="count", title="Heatmap of regional locations of event attendances excluding London")
...
Most of the correlations in this kind of data aren't very strong by statisticians' standards: +/- 0.4 is usually the threshold for "moderate" strength, although I think for exploratory purposes anything from 0.25 upwards *can* be of interest. (For reference, the Jessie MacGregor-Rosa Wallis link scores 0.25.)
